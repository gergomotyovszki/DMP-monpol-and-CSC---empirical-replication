...
VAR.res = Y-[X ones(length(X),1)]*VAR.bet; % unrestricted model
...
VARA.res = VAR.res-[ones(length(VAR.m),1) VAR.m]*Phib; % restricted model
Phib = Phib(2:end,:);
...
RSS1 = VAR.res(:,1)'*VAR.res(:,1);
RSS2 = VARA.res(:,1)'*VARA.res(:,1);
%Fval = (RSS1-RSS2)/RSS1*(VAR.T-VAR.n*VAR.p-1); % F statistic
%Fval = (RSS1-RSS2)/RSS2*(VAR.T-VAR.p-2)
%VAR.Fval = (RSS1-RSS2)/RSS2*(size(VAR.vars,1)-(VAR.p+1)-1);
VAR.Fval = (RSS1-RSS2)/RSS2*(VAR.T-2);
...
% irs(VAR.p+1,:) = -Phib1/Phib1(1,1); % normalize to 1
 for jj=2:VAR.irhor
